...
fn main() -> ! {
    ...
    hprintln!("Starting Pulses...");

    let (pulse_amplitude, pulse_duration): (f32, u32) = (0.5, 20);
    let (read_amplitude, read_duration): (f32, u32) = (0.1, 1);

    let span: (f32, f32) = (-5.0, 5.0);

    let mut measurements: [(usize, [f32; 4]); 500] = core::array::from_fn(|i| (i, [0.0; 4]) );

    for cycle in 0..5 {
        for (i, polarity) in [1.0, -1.0].iter().enumerate() {
            for pulse in 0..50 {
                dac.complete(0, (polarity * pulse_amplitude).clamp(-1.0, 1.0));
                dac.complete(1, (polarity * pulse_amplitude).clamp(-1.0, 1.0));
                dac.complete(2, (polarity * pulse_amplitude).clamp(-1.0, 1.0));
                dac.complete(3, (polarity * pulse_amplitude).clamp(-1.0, 1.0));

                delay.delay_ms(pulse_duration);

                dac.zero();

                dac.complete(0, (polarity * read_amplitude).clamp(-1.0, 1.0));
                dac.complete(1, (polarity * read_amplitude).clamp(-1.0, 1.0));
                dac.complete(2, (polarity * read_amplitude).clamp(-1.0, 1.0));
                dac.complete(3, (polarity * read_amplitude).clamp(-1.0, 1.0));

                delay.delay_ms(read_duration);

                adc.sample(&mut delay);

                dac.zero();

                measurements[(pulse + (50 * i)) + (cycle * 100)] = (
                    (pulse + (50 * i)) + (cycle * 100),
                    [
